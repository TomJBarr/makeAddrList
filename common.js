...
const common = module.exports = {
	...
	if (numberStr.startsWith('0x')) {
	    ...
	    //console.error('numberToBN: expStr =' + numberStr.substring(expIdx + 2));
	    const exp = parseInt(numberStr.substring(expIdx + 2));
	    //console.error('numberToBN: exp = ' + exp);
	    let begPart = numberStr.substring(0, expIdx);
	    //console.error('numberToBN: begPart =' + begPart);
	    let endPart = '';
	    ...
	    //console.error('numberToBN: begPart =' + begPart);
	    //console.error('numberToBN: endPart =' + endPart);
	    numberStr = begPart + endPart
	}
	//console.error('numberToBN: converted from ' + number + ' to ' + numberStr);
	const bn = new BN(numberStr, base);
	//console.error('numberToBN: converted from ' + number + ' to 0x' + bn.toString(16) + ', ' + bn.toString(10));
	return(bn);
    },
	...
	//console.error('common.fetch: fetching ' + url);
	const options = { mode: 'cors'};
	Object.assign(options, extraOptions);
	fetch(url, options).then(function(resp) {
	    //console.error('common.fetch: got resp = ' + resp + ', status = ' + resp.status + ', (' + resp.statusText + ')');
	    clearTimeout(fetch_timer);
	    complete = true;
	    if (timeout == true) {
		console.error("common.fetch: fetch returned after timeout! url = " + url);
		return;
	    }
	    if (resp.ok) {
		...
		console.error("common.fetch: got err = " + resp.blob());
		callback("", "unknown");
	    }
	...
	const timer = setInterval(function() {
		...
		common.web3.eth.getTransactionReceipt(txid, function(err, receipt) {
			...
			console.error('transaction is in block ' + (!!receipt ? receipt.blockNumber : 'err'));
