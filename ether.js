...
function send_guts(acct, key, to_addr, size, size_is_finney, gas_limit, data, broadcast_fcn, callback) {
	...
	ether.get_gas_price(function(gas_price) {
		...
		console.error("send_guts: retrieved nonce, " + nonce + " is less than historical nonce, " + nonce_history + " -- using historical nonce...");
...
//
// following is to serialize and pace calls to getLogs, to ensure that we don't overwhelm the provider node
// (and more importantly, to ensure that we don't get grey-listed)
//
var getLogsList = [];
function GetLogsInfo(options, cb) {
    this.options = options;
    this.cb = cb;
}

// cb(err, result)
function serialGetLogs(options, cb) {
    const getLogsInfo = new GetLogsInfo(options, cb);
...
function getLogsGuts(options, cb) {
	...
	console.error('ether.getLogs: paramsStr = ' + paramsStr);
	...
	console.error('ether.getLogs: body = ' + body);
    }
    common.fetch(url, options, function(str, err) {
	if (!str || !!err) {
	    if (!err)
		err = 'getLogs: error retreiving events';
	    console.error('ether.getLogs: ' + err);
	    cb(err, '');
	    return;
	}
	//console.error('ether.getLogs: err = ' + err + ', str = ' + str);
